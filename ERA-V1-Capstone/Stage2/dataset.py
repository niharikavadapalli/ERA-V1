...
class LlavaCollator(object):
    ...
    def __call__(self, instances: Sequence[dict]) -> dict[str, torch.Tensor]:
        ...
        ans_tokenized = torch.nn.utils.rnn.pad_sequence(ans_tokenized,
                                                 batch_first=True,
                                                 padding_value=self.tokenizer.eos_token_id)
        ques_tokenized = ques_tokenized[:, :self.max_length]
        ans_tokenized = ans_tokenized[:, :self.max_length]

        ques = ques_tokenized.squeeze()
        #print(ques.shape)
        if(len(ques) < self.max_length):
            pad = torch.tensor([self.tokenizer.eos_token_id]*(self.max_length-len(ques))).unsqueeze(0)
            ques_tokenized = torch.cat([ques_tokenized, pad], dim=1)

        ans = ans_tokenized.squeeze()
        #print(ans.shape)
        if(len(ans) < self.max_length):
            pad = torch.tensor([self.tokenizer.eos_token_id]*(self.max_length-len(ans))).unsqueeze(0)
            ans_tokenized = torch.cat([ans_tokenized, pad], dim=1)
        
        #print(ques_tokenized.shape, ans_tokenized.shape)

        batch = dict(
            ques_tokenized=ques_tokenized,
            ans_tokenized=ans_tokenized,
            attention_mask=ques_tokenized.ne(self.tokenizer.eos_token_id),
            question = questions,
        )
